...
def train():
    ...
    for m, batch in enumerate(dataset_train):
        ...
        # print(pred)
        pred = pred.view(batch[0].size(0), n_class)
...
if __name__ == '__main__':
    ...
    sample_data_file = 'E:/2022慕课评论文本挖掘/' + dataname + '.txt'
    label_data_file = 'E:/2022慕课评论文本挖掘/' + dataname + '_label.txt'
    samples = list(open(sample_data_file, "r", encoding='utf-8', errors='ignore').readlines())
    samples = [s.strip() for s in samples]
    labels = list(open(label_data_file, "r", encoding='utf-8', errors='ignore').readlines())
    ...
    with open('E:/2022慕课评论文本挖掘/model/label_to_index_bert_for_' + dataname + '.txt', 'w', encoding='utf-8') as f:
    ...
    for i in range(k):
        ...
        for j in range(len(classifier_list)):
            ...
            for epoch in range(epoches):
                ...
                if acc > best_val_acc:
                    torch.save(bc.state_dict(), 'E:/2022慕课评论文本挖掘/model/model_params_best_{data}_{modelname}_{k_num}.pkl'.format(data=dataname, modelname=classifier_list[j], k_num=i))
                    best_val_acc = acc
                    best_val_auc = auc
                    best_val_mavg = mavg
                    best_val_mae = mae
            ...
            teststart = time.time()
            bc.load_state_dict(torch.load('E:/2022慕课评论文本挖掘/model/model_params_best_{data}_{modelname}_{k_num}.pkl'.format(data=dataname, modelname=classifier_list[j], k_num=i)))
    ...
    for i in range(len(measures_total_avg)):
        ...
        sheet.write(i + 1, 11, measures_total_avg[i][5])
        sheet.write(i + 1, 12, measures_total_std[i][5])
    book.save('E:/2022慕课评论文本挖掘/' + dataname + '_bert_criteria.xls')
